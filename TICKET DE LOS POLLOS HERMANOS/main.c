...
struct Ticket {      //se crea una variable struct Ticket llamada ticket para almacenar los detalles del boleto.
    char sala[10];
    char asiento[10];
    char hora[10];
    char fecha[10];
...
int main() {
    FILE *fp;
    struct Ticket ticket;    //Luego, se solicita al usuario que ingrese los detalles del boleto utilizando la función fgets()
    ...
    printf("Ingresa la Hora de la Función: ");
    fgets(ticket.hora, sizeof(ticket.hora), stdin);
    ticket.hora[strcspn(ticket.hora, "\n")] = '\0';

    printf("Ingresa la Fecha de la función: ");
    ...
    printf("Tu ticket ha sido impreso, disfrute su película.txt\n");
